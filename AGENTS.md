...
### Performance gate: ≥ 2× every competitor

UltraSQL ships only when it outperforms every listed competitor on
every benchmark by a factor of ≥ 2 (throughput) or ≤ 0.5 (latency).
The pre-push hook enforces this via `regression-gate`. There is no
"close enough" — if the floor is missed, the commit is rejected and
the contributor must optimise the relevant kernel.
