...
- [x] `recovery_target_time`, `recovery_target_xid` — startup parses
  `recovery.targets` and WAL recovery stops on commit boundaries by target
  timestamp or transaction ID instead of silently replaying past the target
...
### Logical Replication
- [x] Logical decoding infrastructure — same-process publication registry,
  commit-gated statement CDC stream, durable logical slot metadata, and slot
  confirmed-LSN advancement exist for published tables. WAL row-image decoding
  and network streaming remain tracked by the `pgoutput`/subscription-worker
  targets below.
...
- [x] Date/Time: now(), current_timestamp, current_date, age(),
