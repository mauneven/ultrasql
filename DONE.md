...
- Persistent catalog bootstrap now rejects restored `u32::MAX` object OIDs with
  ...
  `cargo clippy -p ultrasql-catalog --lib --all-features -- -D clippy::unwrap_used -D clippy::expect_used`,
  and `cargo clippy -p ultrasql-catalog --all-targets --all-features -- -D warnings`.
- Role DDL OID allocation now rejects exhausted durable role snapshots with
  `CatalogError::SchemaConflict` instead of wrapping the generated role OID
  after `u32::MAX`. Evidence:
...
- Empty scalar ranges now stay empty across indexable predicate normalization,
  B-tree/BRIN probing, and SSI overlap checks. Strict-bound overflow such as
  `id > i64::MAX` no longer becomes an unbounded range, and impossible
  serializable predicates no longer create false read/write conflicts. Evidence:
  `cargo test -p ultrasql-server match_simple_comparison_clamps_strict_overflow_to_empty_range --lib -- --nocapture`,
  `cargo test -p ultrasql-txn empty_column_range_predicate_matches_nothing --lib -- --nocapture`,
  `cargo test -p ultrasql-server serializable::tests::serializable_read_locks_skip_empty_supported_ranges --lib -- --nocapture`,
  `cargo test -p ultrasql-server serializable::tests::serializable_delete_conflicts_skip_empty_supported_ranges --lib -- --nocapture`,
...
- XML XPath now supports bounded scalar `local-name(/supported/path)`,
  ...
  and
  `cargo test -p ultrasql-server --test xml_round_trip xml_functions_validate_securely_and_extract_simple_xpath -- --nocapture`.
- XML XPath now supports bounded string predicate
  `contains(/supported/path, "literal")` and
  `starts-with(/supported/path, "literal")` functions with quote-aware literal
  argument parsing and no external XML access. Evidence:
  `cargo test -p ultrasql-core xml_xpath_subset_filters_children_without_entity_resolution --lib -- --nocapture`
  and
  `cargo test -p ultrasql-server --test xml_round_trip xml_functions_validate_securely_and_extract_simple_xpath -- --nocapture`.
- XML XPath now supports bounded string slicing
  ...
  and
  `cargo test -p ultrasql-server --test xml_round_trip xml_functions_validate_securely_and_extract_simple_xpath -- --nocapture`.
- XML XPath now supports bounded `substring(path-or-literal, start[, length])`
  over the existing secure selector with XPath 1-based rounded character
  positions and no external XML access. Evidence:
  `cargo test -p ultrasql-core xml_xpath_subset_filters_children_without_entity_resolution --lib -- --nocapture`
  and
  `cargo test -p ultrasql-server --test xml_round_trip xml_functions_validate_securely_and_extract_simple_xpath -- --nocapture`.
- XML XPath now supports bounded `translate(path-or-literal, from, to)` over
  the existing secure selector with XPath first-match replacement/removal
  semantics and no external XML access. Evidence:
  `cargo test -p ultrasql-core xml_xpath_subset_filters_children_without_entity_resolution --lib -- --nocapture`
  and
  `cargo test -p ultrasql-server --test xml_round_trip xml_functions_validate_securely_and_extract_simple_xpath -- --nocapture`.
- XML XPath now supports bounded `concat(path-or-literal, ...)` over path
  first-string values and quoted string literals. Evidence:
  `cargo test -p ultrasql-core xml_xpath_subset_filters_children_without_entity_resolution --lib -- --nocapture`
  and
  `cargo test -p ultrasql-server --test xml_round_trip xml_functions_validate_securely_and_extract_simple_xpath -- --nocapture`.
- XML XPath now supports bounded numeric `number(/supported/path)`,
  `floor(/supported/path)`, and `ceiling(/supported/path)` functions with
  PostgreSQL-compatible `NaN` output for missing/non-numeric paths. Evidence:
  `cargo test -p ultrasql-core xml_xpath_subset_filters_children_without_entity_resolution --lib -- --nocapture`
  and
  `cargo test -p ultrasql-server --test xml_round_trip xml_functions_validate_securely_and_extract_simple_xpath -- --nocapture`.
- XML XPath now supports bounded numeric `round(/supported/path)` with
  PostgreSQL-compatible half-up/toward-positive-infinity behavior. Evidence:
  `cargo test -p ultrasql-core xml_xpath_subset_filters_children_without_entity_resolution --lib -- --nocapture`
  and
  `cargo test -p ultrasql-server --test xml_round_trip xml_functions_validate_securely_and_extract_simple_xpath -- --nocapture`.
- XML XPath now supports bounded numeric node-set `sum(/supported/path)`, with
  empty-set `0` and `NaN` propagation for non-numeric values. Evidence:
  `cargo test -p ultrasql-core xml_xpath_subset_filters_children_without_entity_resolution --lib -- --nocapture`
  and
  `cargo test -p ultrasql-server --test xml_round_trip xml_functions_validate_securely_and_extract_simple_xpath -- --nocapture`.
- XML XPath now supports boolean literal functions `true()` and `false()`.
  Evidence:
  `cargo test -p ultrasql-core xml_xpath_subset_filters_children_without_entity_resolution --lib -- --nocapture`
  and
  `cargo test -p ultrasql-server --test xml_round_trip xml_functions_validate_securely_and_extract_simple_xpath -- --nocapture`.
...
- `TIMESTAMP AT TIME ZONE` and `TIMESTAMPTZ AT TIME ZONE` now bind and execute
  ...
  `cargo test -p ultrasql-server --test timetz_round_trip timetz_and_temporal_display_round_trip -- --nocapture`.
- Fixed-offset and abbreviation `TIMETZ AT TIME ZONE` now binds and executes
  through the same checked runtime path, wrapping across midnight and returning
  `TIMETZ` in the target offset. IANA `TIMETZ` conversion remains open because
  a date anchor is needed for DST-safe offsets. Evidence:
  `cargo test -p ultrasql-planner binds_at_time_zone_with_timestamp_return_types --lib -- --nocapture`;
  `cargo test -p ultrasql-executor date_json_xml_bit_and_network_helpers_cover_scalar_edges --lib -- --nocapture`;
  `cargo test -p ultrasql-server --test timetz_round_trip timetz_and_temporal_display_round_trip -- --nocapture`.
...
- XML scalar functions now cover local-only secure well-formed checks
  ...
  `//` abbreviation, boolean `true()` / `false()`, bounded `count()` /
  `string()` / `boolean()` / `not()` / `name()` / `local-name()` /
...
- The full active public regression subset is now executed by a cargo test,
  not only checked for provenance. Current coverage is 9 shards / 144
...
- Full benchmark certification now includes the active public SQL regression
  subset as `sql-regression`; targeted local run passed 9 shards / 144 cases
...
- Expression/predicate regression baseline covers `IS [NOT] DISTINCT FROM`,
  ...
  `NULLIF`, `GREATEST`, `LEAST`, and `BETWEEN SYMMETRIC`. This also fixed
  `IS DISTINCT FROM` binding/execution, boolean truth-test binding/execution,
  `LIKE` matching to use a bounded dynamic-programming matcher with PostgreSQL
  default backslash escaping for `%` and `_`, and filter fast-path compaction so
  ...
  `cargo test -p ultrasql-executor eval::tests::is_boolean_predicates_use_sql_truth_table --lib -- --nocapture`,
  `cargo test -p ultrasql-executor eval::tests::like --lib -- --nocapture`,
  and `cargo test -p ultrasql-executor filter_op::tests::select_column_preserves_selected_nulls --lib -- --nocapture`.
...
- Join/set-operation regression baseline covers deterministic inner join, left
  ...
  JOIN`, full `USING` join key coalescing, correlated `EXISTS`, `UNION`,
  ...
  emits the planned `Int64` result type instead of leaking `Int32`, the grouped
  hash-aggregate vectorized path so all-null `SUM` groups survive as `NULL`
  instead of being dropped, and `USING`/`NATURAL` join output projection so
  shared columns collapse before outer `SELECT` expressions read row positions
  and full/right joins coalesce unmatched right keys. Evidence:
  ...
  `cargo test -p ultrasql-executor physical::tests::join_using_projects_common_column_once_before_select_exprs --lib -- --nocapture`,
  and `cargo test -p ultrasql-executor physical::tests::full_join_using_coalesces_unmatched_right_key --lib -- --nocapture`.
...
- Runtime float casts now support numeric column/expression casts into `REAL`
  ...
  and
  `cargo test -p ultrasql-server --test core_type_surface_round_trip core_scalar_types_round_trip_over_postgres_wire -- --nocapture`.
- Runtime text casts now parse textlike column/expression values into integer,
  floating, and boolean scalar targets with invalid-input errors instead of a
  blanket non-literal CAST rejection. Evidence:
  `cargo test -p ultrasql-executor cast_size_and_array_error_edges_cover_scalar_compat_paths --lib -- --nocapture`
  and
  `cargo test -p ultrasql-server --test core_type_surface_round_trip core_scalar_types_round_trip_over_postgres_wire -- --nocapture`.
- Runtime temporal text casts now parse textlike column/expression values into
  ...
  and
  `cargo test -p ultrasql-server --test core_type_surface_round_trip core_scalar_types_round_trip_over_postgres_wire -- --nocapture`.
- Runtime zoned temporal text casts now parse textlike column/expression values
  into `TIMESTAMPTZ` and `TIMETZ`, reusing the core fixed-offset,
  abbreviation, and named-zone parsers. Evidence:
  `cargo test -p ultrasql-executor cast_size_and_array_error_edges_cover_scalar_compat_paths --lib -- --nocapture`
  and
  `cargo test -p ultrasql-server --test core_type_surface_round_trip core_scalar_types_round_trip_over_postgres_wire -- --nocapture`.
...
- Runtime errors inside `ON CONFLICT DO UPDATE WHERE` predicate evaluation now
  preserve their typed SQLSTATEs and leave the existing row unchanged.
  Evidence:
...
- Runtime errors inside `ORDER BY` expression-key evaluation now preserve their
  typed SQLSTATEs instead of collapsing to `XX000`. Evidence:
  `cargo test -p ultrasql-server --test order_by_round_trip order_by_runtime_cast_error_returns_22p02 -- --nocapture`.
...
- Runtime errors inside window partition/order/value/default expression
  evaluation now preserve their typed SQLSTATEs instead of collapsing to
  `XX000`. Evidence:
  `cargo test -p ultrasql-server --test window_round_trip window_order_key_runtime_cast_error_returns_22p02 -- --nocapture`.
- Runtime errors inside hybrid-search metadata and WHERE filter evaluation now
...
- TPC-H direct-load Q3 sidecar revenue now rejects malformed discount factors
  and checked revenue accumulation overflow instead of panicking or wrapping
  cached revenue. Evidence:
  `cargo test -p ultrasql-bench --features sql-bench tpch::load::tests --lib -- --nocapture`.
- TPC-H direct-load Q5 sidecar revenue now rejects malformed discount factors
...
- TPC-H direct-load Q7 sidecar revenue now rejects malformed discount factors
  ...
  `cargo test -p ultrasql-bench --features sql-bench tpch::load::tests --lib -- --nocapture`.
- TPC-H direct-load Q8 sidecar market-share volume now rejects malformed
...
- TPC-H direct-load Q10 returned-customer sidecar revenue now rejects malformed
  ...
  `cargo test -p ultrasql-bench --features sql-bench tpch::load::tests --lib -- --nocapture`.
- TPC-H direct-load Q11 German supplier value sidecar now rejects supply-cost
  value overflow and checked part/total value accumulation overflow instead of
  panicking or wrapping cached values. Evidence:
  `cargo test -p ultrasql-bench --features sql-bench tpch::load::tests --lib -- --nocapture`.
