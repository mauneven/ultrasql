...
- Role catalog restart persistence is covered by `pg_auth.meta`: `CREATE ROLE`,
  ...
  instead of silently applying last-row-wins state. Evidence:
  `cargo test -p ultrasql-server --test role_ddl_round_trip role_metadata_rejects_duplicate -- --nocapture`.
