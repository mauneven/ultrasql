...
python3 scripts/run-driver-release-evidence.py \
  --python /tmp/ultrasql-driver-cert/bin/python \
...
| 81 Operator soak reports | `scripts/run-operator-soak.py`, `scripts/validate-operator-soak.py`, `.github/workflows/operator-soak.yml` | `tests/scripts/test_run_operator_soak.py`, `tests/scripts/test_validate_operator_soak.py`, `crates/ultrasql-bench/tests/release_hardening.rs` | no benchmark; 30-day external operation gate | `docs/OPERATOR_SOAK.md`, `docs/operator-reports.md` | `benchmarks/results/latest/operator_soak_status.json` |
