...
Status: schema, helper SQL, and database-enforced tenant RLS for the documented
policy shape. UltraSQL enforces the visible policy form generated by the RAG
helpers: `tenant_id = current_setting('ultrasql.tenant_id', true)`.
This is the first database-enforced tenant RLS slice; broader RLS combinations
remain intentionally outside the claim below.
...
tenant predicates, and visible row-policy SQL. No RAG helper hides
...
Generated policy SQL follows the visible RLS model:
...
- Command-specific policies are honored for scans, `INSERT ... VALUES`,
  `INSERT ... SELECT`, and `UPDATE` new-row checks.
- Restrictive policies narrow permissive policy matches.
- Role-scoped policies honor inherited roles.
- `INSERT` and `UPDATE` rows are checked against applicable `WITH CHECK`
  clauses and reject mixed-tenant mutations atomically.
...
- `benchmarks/rls_tenant_certify.sh` writes the release artifact at
...
Broader RLS combinations remain open beyond this documented tenant pattern.
...
Do not claim full RLS breadth yet. The honest claim is: database-enforced RAG
tenant isolation is available for the documented tenant predicate shape and
covered by wire round-trip tests plus a release certification artifact.
