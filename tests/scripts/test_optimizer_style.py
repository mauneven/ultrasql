...
class OptimizerStyleTests(unittest.TestCase):
    ...
    def test_pg_statistic_uses_checked_catalog_conversions(self) -> None:
        offenders: list[str] = []
        for line_no, line in enumerate(PG_STATISTIC.read_text().splitlines(), start=1):
            code = line.split("//", maxsplit=1)[0]
            if PG_STATISTIC_CAST.search(code):
                offenders.append(f"{PG_STATISTIC.relative_to(REPO)}:{line_no}: {line.strip()}")
