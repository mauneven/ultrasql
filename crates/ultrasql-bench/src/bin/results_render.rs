...
/// Sort engines within a workload best-to-worst (ascending median_us),
/// assign ranks starting from 1.  Ties share the same rank.
...
fn render_workload_md(workload: &str, results: &[EngineResult]) -> String {
    ...
    out.push_str("| Rank | Engine | Median time | Samples |\n");
    out.push_str("|------|--------|-------------|--------|\n");
...
fn render_md(by_workload: &HashMap<String, Vec<EngineResult>>) -> String {
    ...
    out.push_str(
        "> **Note**: UltraSQL rows are intentionally absent until the \
         wire-protocol bench driver (`cross_compare_sql`, Wave 3) records \
         end-to-end measurements through `psql`/libpq. Existing storage- and \
         vectorized-kernel drivers do not exercise the parser, planner, or \
         wire stack, so their numbers are not published alongside the \
         competitors' full-SQL measurements. See \
...
mod tests {
    ...
    /// Engines must be sorted ascending by median_us after ranking.
    #[test]
    fn rank_engines_sorts_ascending() {
        ...
        assert_eq!(results[0].engine, "fast");
        assert_eq!(results[1].engine, "slow");
    }
    ...
    /// When two engines are present, ranks must be 1 and 2 in ascending order.
    #[test]
    fn rank_order_in_json() {
        ...
        assert_eq!(rank1["engine"], "ultrasql");
        assert_eq!(rank2["rank"], 2);
        assert_eq!(rank2["engine"], "postgres");
    }
}
