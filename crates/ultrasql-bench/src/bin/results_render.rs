...
fn render_md(by_workload: &HashMap<String, Vec<EngineResult>>) -> String {
    ...
    out.push_str(
        "> **Methodology**: every row is measured through that engine's full \
         ...
         this matrix as a per-shape microbench, not a whole-product \
...
mod tests {
    ...
    /// When UltraSQL has a lower median than another engine, UltraSQL
    /// must appear first (rank 1, fastest) and the slower engine
    /// last (rank N, slowest).
    #[test]
