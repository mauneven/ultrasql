...
fn render_md(by_workload: &HashMap<String, Vec<EngineResult>>) -> String {
    ...
    out.push_str(
        "> **Note**: UltraSQL rows measure individual kernels in isolation \
         (no SQL surface). Other engines measure their full SQL pipeline. \
         Read every comparison with that context in mind.\n\n",
    );
