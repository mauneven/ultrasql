...
struct DeadlockState {
    ...
    victim: AtomicBool,
    /// Tags this XID has been granted, so [`LockManager::release_all`] visits
    /// only these entries instead of scanning the whole lock table. May hold
    /// duplicates and tags already released individually.
    held: Mutex<Vec<LockTag>>,
}
...
/// Central lock table: one entry per locked object. Keys are generated by
/// the server (relation OIDs, tuple ids, advisory keys), so the faster
/// non-SipHash hasher is safe here.
type LockTable = DashMap<LockTag, Arc<LockEntry>, ahash::RandomState>;

/// Per-transaction deadlock and held-lock state.
type XidStates = DashMap<Xid, Arc<DeadlockState>, ahash::RandomState>;
...
impl LockManager {
    ...
    pub fn with_deadlock_interval(interval: Duration) -> Self {
        let table: Arc<LockTable> = Arc::new(DashMap::with_hasher(ahash::RandomState::new()));
        let xid_states: Arc<XidStates> = Arc::new(DashMap::with_hasher(ahash::RandomState::new()));
    ...
    pub fn release_all(&self, xid: Xid) {
        // Every grant to `xid` was recorded in its state when it was made,
        // so only those entries are visited; the cost does not depend on
        // how many locks other transactions hold.
        let Some((_, xid_state)) = self.xid_states.remove(&xid) else {
            return;
        };
    ...
    fn ensure_xid_state(&self, xid: Xid) -> Arc<DeadlockState> {
