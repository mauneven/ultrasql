...
fn filter_sum_dict_dispatch(xs: &[i64], codes: &[u8], mask: &[i64; 256]) -> i64 {
    ...
    {
        // SAFETY: NEON is part of the AArch64 baseline ABI on every
        // platform we support (`aarch64-apple-darwin` and
        // `aarch64-unknown-linux-gnu` both require ARMv8-A which
        // mandates NEON). The function below uses only unaligned
        // loads bounded by the slice lengths.
        return unsafe { filter_sum_dict_neon(xs, codes, mask) };
    }
...
fn filter_sum_dict_tbl_dispatch(xs: &[i64], codes: &[u8], mask16: &[u8; 16]) -> i64 {
    #[cfg(target_arch = "aarch64")]
    {
        ...
        return unsafe { filter_sum_dict_tbl_neon(xs, codes, mask16) };
    }
...
// ============================================================================
// AArch64 NEON path (general 256-entry dict)
// ============================================================================

/// NEON dict-gather filter+sum, register-resident bit-packed mask.
///
/// The headline optimization: the 256-entry predicate is held entirely
/// in NEON registers as a 32-byte bit-packed table (one bit per code).
/// For each block of 16 codes the kernel resolves all 16 pass/fail
/// decisions with two NEON instructions and no L1 access:
///
/// 1. `vqtbl2q_u8(table32, code >> 3)` gathers 16 bytes of the
///    bit-packed table indexed by `code / 8`. Each byte holds 8
///    consecutive predicate bits.
/// 2. `vshlq_u8(byte, -(code & 7))` per-lane variable right-shift,
///    then `& 1`, isolates the predicate bit for each row.
///
/// We then widen the 16-byte `0/1` vector to 16 `i64` `0/-1` masks via
/// three successive sign-extends — `vmovl_s8`, `vmovl_s16`,
/// `vmovl_s32` — yielding 8 `int64x2_t` mask vectors. AND with the
/// gathered `x` values, accumulate into four independent
/// `int64x2_t` accumulators.
///
/// Total per-iteration cost on M4:
/// - 16 codes loaded via `vld1q_u8` (one 16-byte cache line per 16
///   rows of `y`).
/// - 16 `x` values loaded via four `vld1q_s64_x2` (one cache line per
///   8 rows of `x`).
/// - 2 NEON `tbl/shl/and` instructions for the predicate lookup.
/// - 6 NEON widening sign-extends.
/// - 8 NEON AND + 8 NEON ADD, four-way ILP.
///
/// The predicate path is *not* memory-touching — the 32-byte mask is
/// loaded once before the loop, then lives in NEON registers v0/v1
/// (the table pair argument to `vqtbl2q_u8`). The kernel is bound by
/// the `x` stream's DRAM throughput.
///
/// # Safety
///
/// - NEON is part of the `AArch64` baseline ABI.
/// - All pointer arithmetic is bounded by the loop count; the tail
///   (< 16 lanes) goes through the scalar path.
/// - `vld1q_*` loads require only natural-type alignment, which is
///   satisfied by `i64` slices (8-byte aligned by Rust's allocator).
/// - The `mask_bits` array is 32 bytes; `vld1q_u8` reads 16 bytes
///   from each half — both reads are entirely in-bounds.
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "neon")]
#[allow(clippy::too_many_lines, clippy::similar_names)]
unsafe fn filter_sum_dict_neon(xs: &[i64], codes: &[u8], mask: &[i64; 256]) -> i64 {
    use core::arch::aarch64::{
        int8x16_t, int16x8_t, int32x4_t, int64x2_t, uint8x16_t, uint8x16x2_t, vaddq_s64,
        vaddvq_s64, vandq_s64, vandq_u8, vdupq_n_s8, vdupq_n_s64, vdupq_n_u8, vld1q_s64_x2,
        vld1q_u8, vmovl_high_s8, vmovl_high_s16, vmovl_high_s32, vmovl_s8, vmovl_s16, vmovl_s32,
        vqtbl2q_u8, vreinterpretq_s8_u8, vshlq_u8, vshrq_n_u8, vsubq_s8,
    };

    let n = xs.len().min(codes.len());

    // Derive the 32-byte bit-packed mask from the i64 mask. We do
    // this inline rather than reading from `predicate.mask_bits` so
    // the kernel works against any caller-built `PredicateMask256`
    // (e.g. one whose `mask_bits` was not regenerated after a manual
    // edit). Cost: ≤ 256 byte writes, all hot in L1 before the loop.
    let mut bits = [0_u8; 32];
    for (i, b) in bits.iter_mut().enumerate() {
        let base = i * 8;
        let mut byte: u8 = 0;
        for j in 0..8_usize {
            // SAFETY-of-indexing: i < 32 → base + j < 256, in-bounds
            // of `[i64; 256]`.
            if mask[base + j] != 0 {
                byte |= 1_u8 << j;
            }
    ...
    // SAFETY: 16-byte aligned constant. `vld1q_u8` requires only
    // 1-byte alignment, which `[u8; 32]` trivially satisfies.
    let table: uint8x16x2_t =
        unsafe { uint8x16x2_t(vld1q_u8(bits.as_ptr()), vld1q_u8(bits.as_ptr().add(16))) };

    // Constants.
    let lo3: uint8x16_t = vdupq_n_u8(7);
    let one_u8: uint8x16_t = vdupq_n_u8(1);
    let zero_s8: int8x16_t = vdupq_n_s8(0);

    // Four independent 128-bit accumulators.
    ...
    let x_ptr = xs.as_ptr();
    let c_ptr = codes.as_ptr();

    let chunks = n / 16;
    for k in 0..chunks {
        let off = k * 16;

        // SAFETY: off + 15 < n by construction (k < chunks = n/16),
        // so every read below is in-bounds.
        unsafe {
            // ---- predicate path (register-resident, no memory) ----

            // Load 16 codes.
            let codes_v: uint8x16_t = vld1q_u8(c_ptr.add(off));

            // High 5 bits = byte index into the 32-byte table.
            let byte_idx: uint8x16_t = vshrq_n_u8::<3>(codes_v);
            // Low 3 bits = bit position within the byte.
            let bit_off: uint8x16_t = vandq_u8(codes_v, lo3);

            // Gather 16 bytes from the 32-byte bit-packed table —
            // one byte per row, holding the 8 predicate bits for the
            // code's group.
            let bytes: uint8x16_t = vqtbl2q_u8(table, byte_idx);

            // Per-lane variable right-shift: `vshlq_u8` is a *signed*
            // shift count per lane; a negative count means right-
            // shift. We need bit_off treated as the shift amount, so
            // we negate via `0 - bit_off`. Both operands fit in s8.
            let neg_shift: int8x16_t = vsubq_s8(zero_s8, vreinterpretq_s8_u8(bit_off));
            let shifted: uint8x16_t = vshlq_u8(bytes, neg_shift);

            // Isolate the predicate bit: 0 or 1 per byte.
            let bit: uint8x16_t = vandq_u8(shifted, one_u8);

            // Convert 0/1 to 0/0xFF: subtract from 0 → 0 or -1
            // (in u8 arithmetic that is 0x00 or 0xFF).
            let mask_byte: int8x16_t = vsubq_s8(zero_s8, vreinterpretq_s8_u8(bit));

            // Widen 16 byte-masks (0x00/0xFF) to 16 i64-masks (0/-1)
            // by successive sign-extends. Each widening step doubles
            // the lane width while preserving 0 / -1 values.
            //
            // Helpers used here:
            //   vmovl_s8(vget_low_s8(v))  — sign-extend lanes 0..8
            //   vmovl_high_s8(v)          — sign-extend lanes 8..16
            //   vmovl_s16(vget_low_s16(v)), vmovl_high_s16(v)
            //   vmovl_s32(vget_low_s32(v)), vmovl_high_s32(v)
            let m16_lo: int16x8_t = vmovl_s8(core::arch::aarch64::vget_low_s8(mask_byte));
            let m16_hi: int16x8_t = vmovl_high_s8(mask_byte);

            // Widen i16 (0/0xFFFF) → i32 (0/0xFFFF_FFFF).
            let m32_a: int32x4_t = vmovl_s16(core::arch::aarch64::vget_low_s16(m16_lo));
            let m32_b: int32x4_t = vmovl_high_s16(m16_lo);
            let m32_c: int32x4_t = vmovl_s16(core::arch::aarch64::vget_low_s16(m16_hi));
            let m32_d: int32x4_t = vmovl_high_s16(m16_hi);

            // Widen i32 (0/-1) → i64 (0/-1). Each step doubles lane
            // width and produces two int64x2_t vectors per int32x4_t.
            let m_a0: int64x2_t = vmovl_s32(core::arch::aarch64::vget_low_s32(m32_a));
            let m_a1: int64x2_t = vmovl_high_s32(m32_a);
            let m_b0: int64x2_t = vmovl_s32(core::arch::aarch64::vget_low_s32(m32_b));
            let m_b1: int64x2_t = vmovl_high_s32(m32_b);
            let m_c0: int64x2_t = vmovl_s32(core::arch::aarch64::vget_low_s32(m32_c));
            let m_c1: int64x2_t = vmovl_high_s32(m32_c);
            let m_d0: int64x2_t = vmovl_s32(core::arch::aarch64::vget_low_s32(m32_d));
            let m_d1: int64x2_t = vmovl_high_s32(m32_d);

            // ---- x stream + AND + ADD ----

            let xa = vld1q_s64_x2(x_ptr.add(off));
            let xb = vld1q_s64_x2(x_ptr.add(off + 4));
            let xc = vld1q_s64_x2(x_ptr.add(off + 8));
            let xd = vld1q_s64_x2(x_ptr.add(off + 12));

            acc0 = vaddq_s64(acc0, vandq_s64(xa.0, m_a0));
            acc1 = vaddq_s64(acc1, vandq_s64(xa.1, m_a1));
            acc2 = vaddq_s64(acc2, vandq_s64(xb.0, m_b0));
            acc3 = vaddq_s64(acc3, vandq_s64(xb.1, m_b1));
            acc0 = vaddq_s64(acc0, vandq_s64(xc.0, m_c0));
            acc1 = vaddq_s64(acc1, vandq_s64(xc.1, m_c1));
            acc2 = vaddq_s64(acc2, vandq_s64(xd.0, m_d0));
            acc3 = vaddq_s64(acc3, vandq_s64(xd.1, m_d1));
        }
    }
    ...
    if tail_start < n {
        total = total.wrapping_add(filter_sum_dict_scalar(
            &xs[tail_start..n],
            &codes[tail_start..n],
            mask,
        ));
...
// ============================================================================
// AArch64 NEON path (16-entry dict fast-path)
// ============================================================================

/// NEON `vqtbl1q_u8` fast-path: dict has ≤ 16 entries and the mask
/// fits in a single 16-byte vector register. The table-lookup
/// instruction gathers 16 mask bytes in one cycle, with the table
/// living in a register — no L1 traffic.
///
/// # Safety
///
/// - NEON is part of the `AArch64` baseline ABI.
/// - All loads are bounded by the loop count.
/// - `vqtbl1q_u8` is defined to return 0 for indices ≥ 16; codes are
///   `u8` so we mask them to the low 4 bits before lookup, which
///   matches the documented behaviour of valid `DictI64U8` data with
///   `dict.len() <= 16`.
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "neon")]
#[allow(clippy::similar_names)]
unsafe fn filter_sum_dict_tbl_neon(xs: &[i64], codes: &[u8], mask16: &[u8; 16]) -> i64 {
    use core::arch::aarch64::{
        int8x16_t, int16x8_t, int32x4_t, int64x2_t, uint8x16_t, vaddq_s64, vaddvq_s64, vandq_s64,
        vdupq_n_s64, vget_low_s8, vget_low_s16, vget_low_s32, vld1q_s64_x2, vld1q_u8,
        vmovl_high_s8, vmovl_high_s16, vmovl_high_s32, vmovl_s8, vmovl_s16, vmovl_s32, vqtbl1q_u8,
        vreinterpretq_s8_u8,
    };

    let n = xs.len().min(codes.len());

    // The 16-byte mask: each byte is 0 (fail) or 0xFF (pass). Load
    // once before the loop and keep it in a NEON register across the
    // hot path. `vqtbl1q_u8` reads from this register-resident table,
    // so no memory access is performed for the predicate lookup.
    // SAFETY: 16-byte read from a 16-byte array.
    let tbl: uint8x16_t = unsafe { vld1q_u8(mask16.as_ptr()) };
    ...
    for k in 0..chunks {
        ...
        unsafe {
            // Load 16 codes.
            let codes_v: uint8x16_t = vld1q_u8(c_ptr.add(off));
            // Single-instruction 16-byte table lookup. The result is
            // 16 bytes of 0x00 (fail) or 0xFF (pass), with `vqtbl1q`
            // defined to return 0 for any code byte ≥ 16 — which
            // matches our "codes beyond dict.len() fail" convention.
            let pass_bytes: uint8x16_t = vqtbl1q_u8(tbl, codes_v);

            // Reinterpret as signed bytes for the sign-extend chain.
            // Bytes 0x00 / 0xFF lift to i16 / i32 / i64 values
            // 0 / -1 via `vmovl_s*` widening.
            let mask_byte: int8x16_t = vreinterpretq_s8_u8(pass_bytes);

            // Widen u8 → i16 → i32 → i64 to produce 8 int64x2_t
            // mask vectors covering the 16 lanes.
            let m16_lo: int16x8_t = vmovl_s8(vget_low_s8(mask_byte));
            let m16_hi: int16x8_t = vmovl_high_s8(mask_byte);
            let m32_a: int32x4_t = vmovl_s16(vget_low_s16(m16_lo));
            let m32_b: int32x4_t = vmovl_high_s16(m16_lo);
            let m32_c: int32x4_t = vmovl_s16(vget_low_s16(m16_hi));
            let m32_d: int32x4_t = vmovl_high_s16(m16_hi);
            let m_a0: int64x2_t = vmovl_s32(vget_low_s32(m32_a));
            let m_a1: int64x2_t = vmovl_high_s32(m32_a);
            let m_b0: int64x2_t = vmovl_s32(vget_low_s32(m32_b));
            let m_b1: int64x2_t = vmovl_high_s32(m32_b);
            let m_c0: int64x2_t = vmovl_s32(vget_low_s32(m32_c));
            let m_c1: int64x2_t = vmovl_high_s32(m32_c);
            let m_d0: int64x2_t = vmovl_s32(vget_low_s32(m32_d));
            let m_d1: int64x2_t = vmovl_high_s32(m32_d);
    ...
    let tail_start = chunks * 16;
    if tail_start < n {
        total = total.wrapping_add(filter_sum_dict_tbl_scalar(
            &xs[tail_start..n],
            &codes[tail_start..n],
            mask16,
        ));
