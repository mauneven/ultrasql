...
async fn walsender_handshake_identify_system_and_slot_lifecycle() {
    ...
    // ---- START_REPLICATION is a defined deferred-feature error (Phase 1b) ----
    send_query(&mut stream, "START_REPLICATION SLOT phys_test PHYSICAL 0/0").await;
    let started = drain_to_ready(&mut stream, &mut buf).await;
    assert_eq!(
        error_code(&started).as_deref(),
        Some("0A000"),
        "START_REPLICATION not yet enabled"
    );
