...
fn attnum_overflow_returns_catalog_error() {
    let overflowing_index = usize::try_from(i64::from(i16::MAX)).expect("usize stores i16::MAX");
    let err = attnum_for_index(overflowing_index, "composite type c").expect_err("attnum overflow");
    assert!(
