...
Each competitor line runs its full SQL pipeline through that engine's native
client. **UltraSQL rows are intentionally absent** until the wire-protocol
bench driver lands (`cross_compare_sql`, Wave 3 of the v0.5 sprint): the
existing storage- and vectorized-kernel drivers do not exercise UltraSQL's
parser / planner / wire stack, so publishing those numbers next to full-SQL
competitor numbers would imply parity that does not yet exist. The tables
below show the bar — the competitor numbers UltraSQL targets — and update in
place once honest end-to-end measurements through `psql`/libpq are recorded.
