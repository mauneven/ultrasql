...
def run_ultrasql():
    medians = []
    print("Running UltraSQL stress check (32 iters)...")
    for i in range(20):
        cmd = "cargo run --release -p ultrasql-bench --features='sql-bench' --bin cross_compare_sql -- --workload select-scan --rows 10000 --warmup 2 --iters 32"
...
def run_duckdb():
    medians = []
    print("\nRunning DuckDB stress check (32 iters)...")
    raw_dir = "/tmp/ultrasql-bench-duckdb-stress32"
    ...
    for i in range(20):
        if os.path.exists(json_path):
            os.remove(json_path)
        cmd = f"N_ITERS=32 RAW_DIR={raw_dir} bash benchmarks/scripts/run_duckdb_writes.sh select_scan_10k"
