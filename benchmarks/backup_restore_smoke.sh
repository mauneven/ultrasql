...
DUMP_PATH="$WORK_DIR/backup_restore_smoke.dump"
SOURCE_LOG="$WORK_DIR/source.log"
RESTORE_LOG="$WORK_DIR/restored.log"
...
RESTORE_DSN="postgresql://ultrasql@127.0.0.1:$RESTORE_PORT/ultrasql?sslmode=disable"
...
"$ULTRASQL_BIN" --data-dir "$SOURCE_DATA_DIR" --pg-dump "$DUMP_PATH" --dump-format custom \
    || fail "pg_dump_failed"
"$ULTRASQL_BIN" --data-dir "$RESTORE_DATA_DIR" --pg-restore "$DUMP_PATH" \
    || fail "pg_restore_failed"
"$ULTRASQL_BIN" --data-dir "$RESTORE_DATA_DIR" validate \
    || fail "restore_validate_failed"

RESTORE_PID="$(start_server "$RESTORE_DATA_DIR" "$RESTORE_PORT" "$RESTORE_LOG")"
wait_psql_ready "$RESTORE_DSN" || fail "restored_server_not_ready"

RESTORED_COUNT="$(query_scalar "$RESTORE_DSN" "SELECT COUNT(*) FROM backup_restore_smoke")" \
    || fail "restored_count_failed"
INDEX_RESULT="$(query_scalar "$RESTORE_DSN" "SELECT payload FROM backup_restore_smoke WHERE id = 2")" \
    || fail "restored_index_query_failed"

if [ "$RESTORED_COUNT" != "$SOURCE_COUNT" ]; then
    json_emit "failed" "restored_count_mismatch" "0" "0" "$SOURCE_COUNT" "$RESTORED_COUNT" "$INDEX_RESULT"
    exit 1
fi
if [ "$INDEX_RESULT" != "bravo" ]; then
    json_emit "failed" "restored_index_query_mismatch" "1" "0" "$SOURCE_COUNT" "$RESTORED_COUNT" "$INDEX_RESULT"
    exit 1
fi

json_emit "measured" "ok" "1" "1" "$SOURCE_COUNT" "$RESTORED_COUNT" "$INDEX_RESULT"
echo "backup/restore smoke measured: rows=$RESTORED_COUNT indexed_payload=$INDEX_RESULT"
