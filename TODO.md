...
2. **Predicate-precise SSI / serializable correctness** — column-range SSI degrades to relation-wide
   ...
   (high).
3. **EvalPlanQual / READ COMMITTED re-check on concurrent UPDATE/DELETE** — **PIECE (1) landed**:
   concurrent-writer conflicts are now classified as a retryable SQLSTATE 40001
   (`HeapError::WriteConflict` → `ExecError::SerializationFailure`, commit `626f8d8f`). The
   remaining behavior change — wait + re-evaluate instead of erroring — is split into **PIECE (2)**
   (generalize the proven indexed wait+refresh+re-eval loop to the *scan* int32-pair UPDATE, still
   int32-pair only, behind a hard adversarial gate) and **deferred** SAVEPOINT-class work (DELETE
...
- [ ] EvalPlanQual / READ COMMITTED re-check on concurrent UPDATE/DELETE — on a concurrently-committed row, wait on the row lock then re-evaluate the qual against the new version instead of erroring. **PIECE (1) LANDED** (`626f8d8f`): `HeapError::WriteConflict` is now classified as `ExecError::SerializationFailure` → SQLSTATE 40001 (retryable) via `heap_update_error_to_exec_error` (`crates/ultrasql-executor/src/fused_update.rs:389-396`); it relabels the failure but does not yet change behavior. **PIECE (2)** (the remaining behavior change, gated): generalize the proven indexed wait+refresh+re-evaluate loop (`fused_update.rs:292-348`) to the *scan* int32-pair UPDATE path via an EPQ-mode flag on the scan heap primitives (`update_inplace.rs:676-685`,`:1081-1089`) + `VisiblePreImage` own-vs-foreign disambiguation + savepoint-depth bail + 40P01 deadlock translation. **Deferred** (SAVEPOINT-class, separate gated effort): (a) DELETE EvalPlanQual — DELETE has zero conflict detection today, two in-progress deleters double-stamp `xmax` = a lost-delete (`delete.rs:1755`,`:1810-1834`); (b) general arbitrary-qual/arbitrary-width ModifyTable EvalPlanQual — thread the WHERE `ScalarExpr` out of the child Filter (`lowering.rs:26-53`) into a per-row lock/refetch/re-bind loop, fighting the bulk drain→`update_many`/`delete_many` design (`operator.rs:62-65`,`:472`,`:493`); (c) ctid-chain following for the version-creating `heap.update` path (`helpers.rs:110` `MalformedHeader`), unifying the two `updated_ctid_target` chasers (`btree_probe.rs:484`, `modify/helpers.rs:131`). See [`docs/evalplanqual-design.md`](docs/evalplanqual-design.md) for the full PIECE split, ordered steps, and the adversarial battery that gates PIECE (2) (high; `crates/ultrasql-storage/src/heap/update_inplace.rs:674-685`, `crates/ultrasql-executor/src/fused_update.rs:321-344`).
- [ ] SERIALIZABLE READ ONLY DEFERRABLE + safe-snapshot deferral — `BEGIN` / `SET TRANSACTION` now **parse and honor `READ ONLY` / `READ WRITE`**: a data-modifying statement in a read-only transaction is rejected with SQLSTATE `25006` (covering the general DML path, the fused `(int32,int32)` INSERT fast path, and `COPY … FROM`) and aborts the block, while `nextval` stays permitted — matching PostgreSQL. Still open: `[NOT] DEFERRABLE` is parsed but **inert** (no behavior), there is no read-only-transaction safe-snapshot detection, and a serializable read-only txn is still fully rw-conflict-tracked regardless of its read-only status (medium; `crates/ultrasql-txn/src/ssi.rs:640-655`, `crates/ultrasql-server/src/session/txn.rs` `execute_begin`/`execute_set_transaction`).
